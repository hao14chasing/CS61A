...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def judge(a):
        new_list=split(lower(remove_punctuation(a)))
        for i in topic:
            for p in new_list:
                if i==p:
                    return True
...
def accuracy(typed, reference):
    ...
    if len(typed_words)==0 and len(reference_words)==0:
        ...
        denominator,numerator,i=len(typed_words),0,0
        if len(reference_words)<=len(typed_words):
            typed_words=typed_words[:len(reference_words)+1]
            ...
            reference_words=reference_words[:len(typed_words)+1]
