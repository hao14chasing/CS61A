...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    # END PROBLEM 3
    if num_rolls!=0:
       goal_1=roll_dice(num_rolls, dice)
    else:
       goal_1=oink_points(player_score, opponent_score)
    return goal_1
...
def pigs_on_prime(player_score, opponent_score):
    ...
    if is_prime(player_score)==False:
        ...
        b=1
        while is_prime(player_score+b)==False:
            b+=1
        return b
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0<goal and score1<goal:
        if who==0:
            add1=take_turn(strategy0(score0,score1),score0,score1,dice)
            score0=score0+add1
            add2=pigs_on_prime(score0,score1)
            score0=score0+add2
        elif who==1:
            add3=take_turn(strategy1(score1,score0),score1,score0,dice)
            score1=score1+add3
            add4=pigs_on_prime(score1,score0)
            score1=score1+add4
        who=next_player(who)
    # END PROBLEM 5
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if score0==score1:
       ...
       message=None
         ...
         last_leader=0
         if last_leader==last_s1:
            message=None
         else:
            message='Player '+str(last_leader)+' takes the lead by '+str(score0-score1)
    else:
         last_leader=1
         if last_leader==last_s1:
            message=None
         else: 
            message='Player '+str(last_leader)+ ' takes the lead by '+str(score1-score0)
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    initial=make_averaged(roll_dice,total_samples)(1,dice)
    e,i=1,2
    while i<=10:
        later=make_averaged(roll_dice,total_samples)(i,dice)
        if initial<later:
            e=i
            initial=later
        i+=1
...
def pigs_on_prime_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    if is_prime(score+oink_points(score,opponent_score))>0:
        return 0
    else:
