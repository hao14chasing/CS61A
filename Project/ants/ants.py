...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not next_place.is_hive:
            if self.lower_bound<=limit<=self.upper_bound and next_place.bees:
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if not gamestate.queen1:
            gamestate.queen1 = True
    ...
    def action(self, gamestate):
        ...
        while pos:
            if pos.ant is not None:
                if not pos.ant.has_doubled:
                    pos.ant.has_doubled = True
                    pos.ant.double()
                if pos.ant.is_container and pos.ant.ant_contained is not None:
                    # the pos.ant.ant_contained may change
                    if not pos.ant.ant_contained.has_doubled:
                        pos.ant.ant_contained.double()
                        pos.ant.ant_contained.has_doubled = True
            pos = pos.exit
        # END Problem 12
