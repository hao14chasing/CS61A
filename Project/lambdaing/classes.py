...
class TutorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent):
        ...
        if len(self.player.hand)==0:
            ...
            self.player.hand.append(self.player.hand[0].copy())
        # END Problem 4
        # You should add your implementation above this.
        if added:
...
class TACard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if len(self.player.hand)==0:
            ...
            new_list=([i.power(opponent_card) for i in self.player.hand])
            index=new_list.index(max(new_list))
            best_card=self.player.hand.pop(index)
...
class InstructorCard(Card):
    ...
    def effect(self, opponent_card, player, opponent, arg=None):
        ...
        if self.attack>=0 or self.defense>=0:
            re_add=InstructorCard("Instructor Card", self.attack, self.defense)
            self.player.hand.append(re_add)
        # END Problem 6
        # You should add your implementation above this.
        if re_add:
